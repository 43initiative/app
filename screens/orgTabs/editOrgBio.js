...
import  {updateOrgBio, updateOrgProfilePic} from "../../firebase/fireStarter";
import {UPDATE_ORG_BIO, UPDATE_ORG_IMG} from "../../reducers/actionTypes";


export default class EditOrgBio extends React.Component {
    constructor(props) {
        super(props);
    ...
    saveAndContinue = async () => {
        activateLoading()
        //update the database
        await updateOrgBio(this.state.bio)
        //update the user data store
        let store = storeControllers.store
        store.dispatch({type:UPDATE_ORG_BIO,payload:this.state.bio})
