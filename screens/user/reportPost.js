...
import {reportPost} from "../../firebase/fireStarter";
...
export default class ReportPost extends React.Component {
    ...
    submitFeedback = async () => {
        ...
        activateLoading()
        let response = await reportPost(this.state.helpFeedback,data.userUid,data.postId)
        if(response.passed) {
                ...
                <SafeAreaView>
                    ...
                    <View style={[{backgroundColor:'white',width:'100%',height:'100%'}]}>
                        ...
                        <Text style={[{color:'black',width:'90%',marginLeft:'5%',marginTop:'2.5%',fontSize:15}]}>
                            Reporting a post is a serious action, please only do so if the post violates our terms, or feeatures an image or video
                            without '43' included in it.
                        </Text>
                    ...
                    <View style={[{position:'absolute',top:'75%',width:'90%',marginLeft:'5%',height:'15%'}]}>
                        <RoundedButton disabled={this.state.helpFeedback === ''} pressed={()=>{this.submitFeedback()}} style={[{width:'90%',marginLeft:'5%',height:'50%'}]} bgColor={'#3EB489'} text={'Report Post'}/>
